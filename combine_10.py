...
with st.sidebar:
    ...
    json_dir = st.text_input("📂 JSON Directory")
    image_dir = st.text_input("🖼 Image Directory")
    ...
    if json_dir and image_dir:
        ...
        if not json_dir_path.exists() or not json_dir_path.is_dir():
            ...
            raw_json_files = [f.strip() for f in os.listdir(json_dir) if f.endswith(".json")]
            # Use natsorted to sort naturally (e.g., page_2 comes before page_10)
            ...
            # Initialize current_json_idx if not already set
            ...
            selected_json = st.selectbox("📜 Select JSON File", 
...
# Use natsorted to sort the files naturally (e.g., page_2 before page_10)
sorted_json_files = natsorted(raw_json_files)


# Get available images
image_files = sorted([f for f in os.listdir(image_dir) if f.endswith(".png")])
# Automatically select corresponding image based on JSON filename
...
# If json_data is a list, search for the dictionary with page dimensions
if isinstance(json_data, list):
    page_info = next(
        (item for item in json_data if isinstance(item, dict) and "page_width" in item and "page_height" in item),
        {}
    )
else:
    page_info = json_data
...
# Create two-column layout for further display/annotation
