#here in this streamlit app has move up/down function,creates a canvas and makes a bounding box which extarcts coords and makes a url , the rendering works are live,show bounding boxes or not on the image is done
#previous /next pdf image or json is done,boxes are made properly/mathpix is integrated and the output is shown with the cropped box and the image downloaded in the download folder can also be sent to the mathpix
...
def get_latest_downloaded_image():
    ...
    image_files = sorted(glob.glob(os.path.join(downloads_folder, "*.png")), key=os.path.getctime, reverse=True)
