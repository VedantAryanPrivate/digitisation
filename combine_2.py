    ...
    if first_entry and "lines" in first_entry:
        for idx, annotation in enumerate(first_entry["lines"]):
            with st.expander(f"Box {idx}", expanded=False):
                ...
                new_x = st.number_input(f"X {idx}", value=x)
                new_y = st.number_input(f"Y {idx}", value=y)
                new_w = st.number_input(f"Width {idx}", value=w)
                new_h = st.number_input(f"Height {idx}", value=h)
                new_text = st.text_area(f"Text {idx}", annotation["text"])

                col_up, col_down = st.columns(2)
                with col_up:
                    if st.button(f"⬆️ Move Up {idx}") and idx > 0:
                        first_entry['lines'][idx - 1], first_entry['lines'][idx] = first_entry['lines'][idx], first_entry['lines'][idx - 1]
                        with open(json_path, "w", encoding="utf-8") as f:
                            json.dump(json_data, f, indent=4)
                        st.experimental_rerun()
                with col_down:
                    if st.button(f"⬇️ Move Down {idx}") and idx < len(first_entry['lines']) - 1:
                        first_entry['lines'][idx + 1], first_entry['lines'][idx] = first_entry['lines'][idx], first_entry['lines'][idx + 1]
                        with open(json_path, "w", encoding="utf-8") as f:
                            json.dump(json_data, f, indent=4)
                        st.experimental_rerun()
